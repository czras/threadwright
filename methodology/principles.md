# Threadwright Principles

Threadwright is guided by principles that shape how it is applied, interpreted, and evolved.

These principles are not a prescribed process. They provide constraints and guidance for making decisions when the methodology does not prescribe a specific answer.

They are general and cross-cutting. Where a principle already has a corresponding structural rule or practical heuristic elsewhere in the methodology, that relationship is noted rather than restated.

## Optimize for the Objective

Do not confuse the objective being optimized with a convenient proxy for that objective.

A measurable activity, output, or intermediate result is not necessarily the outcome that matters.

Threadwright should remain oriented toward consequential knowledge, meaningful uncertainty reduction, and the intended outcome rather than optimizing proxies that merely correlate with them. This guards the [I5 — Optimization](threadwright.md#i5--optimization) invariant against drifting toward whatever is easiest to measure.

This principle is currently an emerging hypothesis, not established knowledge — see [ASSESSMENT-0010](artifacts/ASSESSMENT-0010_objective-and-proxy-in-knowledge-optimization.md) for its origin and open validation questions.

## Address Consequential Gaps

Prioritize gaps according to their consequence for the intended outcome.

Not every unknown needs to be resolved. Not every uncertainty warrants investigation. Not every missing piece of information is equally valuable to obtain.

The value of an activity depends on how materially it can improve the ability to achieve the intended outcome. Applied through [H1 — Address Consequential Gaps](heuristics/address-consequential-gaps.md).

## Generate Broadly, Compress Deliberately

When generation is cheap, generate broadly before selecting what should remain.

Broad generation can expose possibilities, relationships, and structures that would otherwise remain undiscovered. Selection and compression should then remove cognitive noise, redundancy, and unnecessary complexity without destroying useful information.

Compactness is not minimalism. The objective is not the smallest possible result, but the greatest useful value and comprehension for the cognitive cost. [Methodology Change Review](activities/methodology-change-review.md) applies this when compressing a methodology change.

## Move Toward Reality When Abstraction Stops Producing Knowledge

When reasoning, analysis, or abstraction can no longer reduce a consequential gap effectively, move closer to the context in which reality can generate the missing knowledge.

This may mean building, testing, observing, experimenting, interacting, or otherwise creating experience.

The boundary between thinking and experience is therefore dynamic rather than fixed — see [Dynamic Think/Experience Boundary](heuristics/dynamic-think-experience-boundary.md).

## Keep Complexity Proportional to Evidence

Before creating a new mechanism, abstraction, process, or structure, examine whether an existing one can satisfy the requirement. Use the simplest structure that preserves the required properties.

New complexity should earn its existence through evidence that existing abstractions are insufficient. Do not introduce machinery, or solve future complexity, merely because it is imaginable.

When experience demonstrates that the simpler structure is no longer sufficient, evolve it.

## Preserve Epistemic Boundaries

Distinguish what is observed from what is interpreted, hypothesized, decided, or concluded.

Do not allow generated, inferred, or assumed knowledge to silently acquire the status of established knowledge.

Confidence does not substitute for evidence. This is the [I3 — Epistemic Integrity](threadwright.md#i3--epistemic-integrity) invariant; see [Epistemic Integrity](heuristics/epistemic-integrity.md) for the distinctions it requires.

## Preserve Provenance When It Carries Meaning

When the origin or evolution of an idea materially affects its interpretation, preserve that provenance.

An insight derived from experience should remain distinguishable from a principle that was subsequently generalized from it.

The history of an idea can itself be useful knowledge — see [`foundations.md`](foundations.md) for Threadwright's own provenance.

## Let Experience Change the Method

Threadwright is not expected to remain unchanged.

Its principles, heuristics, structures, and practices may evolve when experience provides consequential evidence that they should.

Changes should therefore be treated as hypotheses about how Threadwright can become more useful, rather than as inherently correct improvements. This is Threadwright's [recursive self-application](threadwright.md#recursive-self-application): it learns from its own use.

## Reassess After Change

A change to Threadwright can affect more than the thing being changed.

After consequential changes, reassess the resulting methodology as a whole: its coherence, conceptual boundaries, relationships, cognitive load, and ability to serve its intended purpose.

A locally correct change can produce globally undesirable effects. This specializes the [I4 — Feedback](threadwright.md#i4--feedback) invariant (see also [H4 — Reassess](heuristics/reassess.md)) for changes to the methodology itself; [Methodology Change Review](activities/methodology-change-review.md) is its operationalization.

---

These principles constrain Threadwright without prescribing a universal workflow. Their application remains contextual and subject to judgment.
